...
export async function GET(request, { params }) {
  ...
  // const responseData = splitTypeData[id];
...
WHERE
	Floor.Id IN ( SELECT Floor.Id FROM Floor JOIN Device ON Floor.Id = Device.FloorId WHERE Device.Id = ${id} AND Room <> 0 ) 
...
GROUP BY 
  ...
  const deviceQuery = `SELECT 	Device.Id AS DeviceId,
	...
	AND DeviceTypeId LIKE '%split_type%' AND Room <> 0 `;
...
ORDER BY [Timestamp] desc`;
    ...
    let responseData = splitTypeDataObject;

    if (responseData) {
      return NextResponse.json(responseData);
    }
  } else return NextResponse.json({ message: "no data" });
}
