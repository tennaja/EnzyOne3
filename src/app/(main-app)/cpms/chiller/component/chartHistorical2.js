...
import { getHistoricalChart2 } from "@/utils/api";
...
export default function HistoricalChart2({ BuildingId }) {
  ...
  async function GetHistoricalGraph(buildingId, dateFrom, dateTo) {
    ...
    const res = await getHistoricalChart2(paramsNav);
    console.log(paramsNav);
    if (res.status === 200) {
      console.log(res.data.flow)
      if (res.data.flow) {
        console.log([res.data.flow])
        setFlowList(res.data.flow);
        let label = [];
        let modday = 0;
        if (res.data.flow[0].data != null){
          for (let j = 0; j < res.data.flow[0].data.length; j++) {
            label.push(res.data.flow[0].data[j].time);
          }
        setListLabel(label);
        console.log(label)
        }    
      ...
      if (res.data.efficiency) {
        console.log(res.data.efficiency)
        setEfficiencyList(res.data.efficiency);
        let label = [];
        let modday = 0;
        if (res.data.efficiency[0].data != null){
          for (let j = 0; j < res.data.efficiency[0].data.length; j++) {
            label.push(res.data.efficiency[0].data[j].time);
          }
          setListLabel(label);
          console.log(label)
        }  
