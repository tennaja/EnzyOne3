...
const CustomGraph = () => {
  ...
  const GetConsumtionDeviceHistory = async (
    chartId,
    ids,
    units,
    startDate,
    ...
    try {
      ...
      if (result && result.status === 200) {
        ...
        setCharts((prev) =>
          prev.map((chart) =>
            chart.id === chartId ? { ...chart, data: combinedData } : chart
          )
        );
    ...
    setCharts((prev) =>
      prev.map((chart) => {
        ...
        const newParams = paramIds
          .filter(
            (id) =>
              !chart.selectedParams.some((p) => p.id === id && p.unit === unit)
          )
          .map((id) => {
        ...
        const uniqueParams = updatedParams.filter(
          (param, index, self) =>
            index ===
            self.findIndex((p) => p.id === param.id && p.unit === param.unit)
        );
                ...
                <LineChart data={chart.data || []}>
                  ...
                  {chart.selectedParams.map((param, index) => (
                    <Line
                      ...
                      dot={false}
                      yAxisId={
                        param.unit === chart.uniqueUnits[0] ? "left" : "right"
                      } // แยกแกน Y ตาม unit
