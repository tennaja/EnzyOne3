...
const SchedulePopup = forwardRef(
  (
    ...
    const [repeatOption, setRepeatOption] = useState(
      scheduleData?.repeat || "please select"
    );
    ...
    const [endDatetime, setEndDatetime] = useState(
      scheduleData?.endTime?.replace(/\s*\(\+\d+\)$/, "") ||
        `${now.getHours().toString().padStart(2, "0")}:${now
          .getMinutes()
          .toString()
          .padStart(2, "0")}`
    ...
    const executionDate = executionDateTime
      ? formatDate(new Date(executionDateTime))
      : null;
    const executionTime = executionDateTime
      ? formatTime(new Date(executionDateTime))
      : null;
    const executionEndDate = executionEndDateTime
      ? formatDate(new Date(executionEndDateTime))
      : null;
    const executionEndTime = executionEndDateTime
      ? formatTime(new Date(executionEndDateTime))
      : null;
    ...
    useImperativeHandle(ref, () => ({
      triggerSave: async () => {
        const param = {
          ...
          dayOfWeek:
            repeatOption === "once" || repeatOption === "please select"
              ? []
        ...
        >
          <div className="p-4 dark:bg-gray-800 border dark:text-white border-gray-600 rounded-md">
            <h2 className="text-xl font-semibold mb-4">
              {action == "create" ? "Add" : "Edit"} Schedule
            </h2>
            <form>
...
/>
                        ...
                        <th
                          className="p-2 text-left !important "
              ...
              <div className="grid grid-cols-[0.5fr_2fr] items-center gap-x-4 mt-3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setHasSelectedStartDate(false); // reset เมื่อเปลี่ยน repeat option
                ...
                {repeatOption === "please select" && (
                  <div className="flex gap-2 mt-2 w-full">
                    <DatePicker
                      ...
                      showTime
                      value={
                        executionDateTime ? dayjs(executionDateTime) : null
                      }
                      onChange={(date) => {
                        setexecutionDateTime(date ? date.toISOString() : null);
                        if (
                          executionEndDateTime &&
                          dayjs(date).isAfter(dayjs(executionEndDateTime))
                        ) {
                    ...
                    <DatePicker
                      ...
                      onChange={(date) => {
                        ...
                        if (
                          ...
                          dayjs(date)
                            .add(1, "minute")
                            .isAfter(dayjs(executionEndDateTime))
                        ) {
                      ...
                      } // กำหนดไม่ให้เลือกวันก่อนปัจจุบัน
                    />
                      ...
                      <TimePicker
                        ...
                        onChange={(time) => {
                          ...
                          if (
                            ...
                            dayjs(time, "HH:mm")
                              .add(1, "minute")
                              .isAfter(dayjs(endDatetime, "HH:mm"))
                          ) {
                            const autoEnd = dayjs(time, "HH:mm")
                              .add(1, "minute")
                              .format("HH:mm");
                            setEndDatetime(autoEnd);
