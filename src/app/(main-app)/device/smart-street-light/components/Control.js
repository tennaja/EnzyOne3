...
import { DeviceControl, getDeviceListData } from "@/utils/api";
...
export default function DeviceControlPage({
  ...
  const handleOpenModalconfirm = () => {
    ...
    <div class="mx-auto w-fit px-4 text-left bg-red">
      <p>Device: ${selecteddeviceData?.length} device${
        selecteddeviceData?.length > 1 ? "s" : ""
      } selected</p>
  ...
  const filtereddeviceData = devcielist.filter((item) => {
    ...
    return (
      ...
      (isNumberSearch
        ? item.kW === Number(searchTerm)
        : item.kW?.toString().toLowerCase().includes(search)) ||
      (isNumberSearch
        ? item.kWh === Number(searchTerm)
        : item.kWh?.toString().toLowerCase().includes(search)) ||
      (isNumberSearch
        ? item.percentDimming === Number(searchTerm)
        : item.percentDimming?.toString().toLowerCase().includes(search)) ||
      (isNumberSearch
        ? item.runningHour === Number(searchTerm)
        : item.runningHour?.toString().toLowerCase().includes(search)) ||
  ...
  const toggleSelectAll = () => {
    const selectableDevices = filtereddeviceData.filter(
      (device) => device.status !== "offline"
    );
    const isAllSelected =
      selecteddeviceData.length === selectableDevices.length;

    const updatedDeviceData = isAllSelected
      ? []
      : selectableDevices.map((device) => device.id);
  ...
  const currentdeviceData = sortedData.slice(
    indexOfFirstDevice,
    indexOfLastDevice
  );
        ...
        <div className="lg:w-3/4 w-full  border-r p-2">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-4 text-sm text-gray-600 dark:text-white">
                        {highlightText(device.description)}
                      </td>
                      <td className="py-2 px-4 text-sm text-gray-600 dark:text-white">
                        {highlightText(device.siteName)}
                      </td>
                      <td className="py-2 px-4 text-sm text-gray-600 dark:text-white">
                        {highlightText(device.groupName)}
                      </td>
                      ...
                      <td className="py-2 px-4 text-sm text-gray-600 dark:text-white text-right">
                        {highlightText(device.lastUpdated)}
                      </td>
                    </tr>
                  );
                })
          ...
          <div className="mt-7 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                disabled={
                  currentPage === totalPages || filtereddeviceData.length === 0
                }
                onClick={() => handlePageChange(currentPage + 1)}
                className="px-2 py-1 text-sm bg-gray-200 dark:text-white rounded-lg disabled:opacity-50"
              >
                ...
                <div className="flex items-center w-80">
                  <div className="w-80 flex items-center justify-between">
                    <div className="w-full">
                      ...
                      <datalist
                        id="tickmarks"
                        className="w-full flex justify-between text-xs text-gray-600 dark:text-white"
                      >
