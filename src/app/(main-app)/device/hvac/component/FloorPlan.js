...
export default function FloorPlan({ FloorId }) {
  ...
  async function getAllData(FloorId) {
    await getfloorplan(FloorId);
    // getAHUList(FloorId);
    // getVAVList(FloorId);
    // await getSplittypeList(FloorId);
    // await getIOTList(FloorId);
  }
  ...
  const getfloorplan = async (floorId) => {
    ...
    console.log("floor plan data: ", result.data);
        ...
        <div className="flex flex-col gap-4 p-2">
          {floorplanList.length > 0 &&
            floorplanList.map((item, index) => {
              console.log("floorplan mapping:", item.name);
              return (
                <div key={item.id}>
